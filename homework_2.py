...
rows, colums = int(input("Введите количество строк: ")), int(input("Введите количество столбцов: "))
op = lambda a, b: a * b

CreateOperation = lambda op, a, b: [[op(x, y) for x in range(1, a+1)] for y in range(1, b+1)]

def print_operation_table(matrix, rows, colums):
    for i in range(rows):
        for j in range(colums):
            print(matrix[j][i], end ="\t")
        print(end = "\n")
    
print_operation_table(CreateOperation(op, rows, colums), rows, colums)